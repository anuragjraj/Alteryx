...
export default async function handler(req, res) {
  // Health check: open /api/explain-alteryx in your browser -> { ok:true, hasKey:true }
  if (req.method === "GET") {
...
If labelled sections are listed, use them as the phases. For each phase, keep steps to the few that matter - do not list every tool. Say plainly what we end up with. Aim for about 5 phases, in order. Keep every value concise and free of symbols.`
  ...
  try {
    ...
    try {
      const r2 = await groq.chat.completions.create({
        model: MODEL, temperature: 0.3, max_tokens: 3000, messages,
      })
      return res.status(200).json({ content: r2.choices?.[0]?.message?.content || "" })
    } catch (e2) {
      console.error("Groq retry error:", e2?.status, e2?.message)
      return res.status(500).json({ error: e2?.message || e?.message || "Groq request failed", status: e2?.status || e?.status || null })
    }
  }
}
